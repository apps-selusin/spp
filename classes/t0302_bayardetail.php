...
/**
 * Table class for t0302_bayardetail
 */
class t0302_bayardetail extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		$this->TableVar = 't0302_bayardetail';
		$this->TableName = 't0302_bayardetail';
		$this->TableType = 'TABLE';

		// Update Table
		$this->UpdateTable = "`t0302_bayardetail`";
		$this->Dbid = 'DB';
		$this->ExportAll = TRUE;
		...
		// id
		$this->id = new DbField('t0302_bayardetail', 't0302_bayardetail', 'x_id', 'id', '`id`', '`id`', 3, -1, FALSE, '`id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// bayarmaster_id
		$this->bayarmaster_id = new DbField('t0302_bayardetail', 't0302_bayardetail', 'x_bayarmaster_id', 'bayarmaster_id', '`bayarmaster_id`', '`bayarmaster_id`', 3, -1, FALSE, '`bayarmaster_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->bayarmaster_id->IsForeignKey = TRUE; // Foreign key field
		$this->bayarmaster_id->Nullable = FALSE; // NOT NULL field
		$this->bayarmaster_id->Required = TRUE; // Required field
		...
		// iuran_id
		$this->iuran_id = new DbField('t0302_bayardetail', 't0302_bayardetail', 'x_iuran_id', 'iuran_id', '`iuran_id`', '`iuran_id`', 3, -1, FALSE, '`iuran_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		$this->iuran_id->Lookup = new Lookup('iuran_id', 't0106_iuran', FALSE, 'id', ["Iuran","","",""], [], [], [], [], [], [], '', '');
		...
		// Keterangan
		$this->Keterangan = new DbField('t0302_bayardetail', 't0302_bayardetail', 'x_Keterangan', 'Keterangan', '`Keterangan`', '`Keterangan`', 200, -1, FALSE, '`Keterangan`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		$this->Keterangan->Sortable = TRUE; // Allow sort
		$this->fields['Keterangan'] = &$this->Keterangan;

		// Jumlah
		$this->Jumlah = new DbField('t0302_bayardetail', 't0302_bayardetail', 'x_Jumlah', 'Jumlah', '`Jumlah`', '`Jumlah`', 4, -1, FALSE, '`Jumlah`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	public function getMasterFilter()
	{
		...
		if ($this->getCurrentMasterTable() == "t0301_bayarmaster") {
			if ($this->bayarmaster_id->getSessionValue() <> "")
				$masterFilter .= "`id`=" . QuotedValue($this->bayarmaster_id->getSessionValue(), DATATYPE_NUMBER, "DB");
			else
				return "";
		}
		return $masterFilter;
	...
	public function getDetailFilter()
	{
		...
		if ($this->getCurrentMasterTable() == "t0301_bayarmaster") {
			if ($this->bayarmaster_id->getSessionValue() <> "")
				$detailFilter .= "`bayarmaster_id`=" . QuotedValue($this->bayarmaster_id->getSessionValue(), DATATYPE_NUMBER, "DB");
			else
				return "";
		}
		return $detailFilter;
	...
	// Master filter
	public function sqlMasterFilter_t0301_bayarmaster()
	{
		return "`id`=@id@";
	}

	// Detail filter
	public function sqlDetailFilter_t0301_bayarmaster()
	{
	...
	{
		return ($this->SqlFrom <> "") ? $this->SqlFrom : "`t0302_bayardetail`";
	}
	...
	public function getReturnUrl()
	{
		...
		if (@$_SESSION[$name] <> "") {
			...
			return "t0302_bayardetaillist.php";
		}
	}
	...
	public function getModalCaption($pageName)
	{
		...
		elseif ($pageName == "t0302_bayardetailedit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "t0302_bayardetailadd.php")
	...
	{
		if ($parm <> "")
			$url = $this->keyUrl("t0302_bayardetailview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("t0302_bayardetailview.php", $this->getUrlParm(TABLE_SHOW_DETAIL . "="));
	...
	{
		if ($parm <> "")
			$url = "t0302_bayardetailadd.php?" . $this->getUrlParm($parm);
		else
			$url = "t0302_bayardetailadd.php";
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("t0302_bayardetailedit.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		$url = $this->keyUrl("t0302_bayardetailadd.php", $this->getUrlParm($parm));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("t0302_bayardetaildelete.php", $this->getUrlParm());
	}

	// Add master url
	public function addMasterUrl($url)
	{
		if ($this->getCurrentMasterTable() == "t0301_bayarmaster" && !ContainsString($url, TABLE_SHOW_MASTER . "=")) {
	...
	public function renderEditRow()
	{
		...
		$this->bayarmaster_id->PlaceHolder = RemoveHtml($this->bayarmaster_id->caption());
		}
