 ...
 */
class t0301_bayarmaster extends DbTable
{
	protected $SqlFrom = "";
	protected $SqlSelect = "";
	...
	public function __construct()
	{
		...
		// id
		$this->id = new DbField('t0301_bayarmaster', 't0301_bayarmaster', 'x_id', 'id', '`id`', '`id`', 3, -1, FALSE, '`id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'NO');
		...
		// Nomor
		$this->Nomor = new DbField('t0301_bayarmaster', 't0301_bayarmaster', 'x_Nomor', 'Nomor', '`Nomor`', '`Nomor`', 200, -1, FALSE, '`Nomor`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// siswa_id
		$this->siswa_id = new DbField('t0301_bayarmaster', 't0301_bayarmaster', 'x_siswa_id', 'siswa_id', '`siswa_id`', '`siswa_id`', 3, -1, FALSE, '`siswa_id`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
		...
		// Total
		$this->Total = new DbField('t0301_bayarmaster', 't0301_bayarmaster', 'x_Total', 'Total', '`Total`', '`Total`', 4, -1, FALSE, '`Total`', FALSE, FALSE, FALSE, 'FORMATTED TEXT', 'TEXT');
	...
	public function getDetailUrl()
	{
		...
		if ($this->getCurrentDetailTable() == "t0302_bayardetail") {
			$detailUrl = $GLOBALS["t0302_bayardetail"]->getListUrl() . "?" . TABLE_SHOW_MASTER . "=" . $this->TableVar;
			$detailUrl .= "&fk_id=" . urlencode($this->id->CurrentValue);
		}
		if ($detailUrl == "")
			$detailUrl = "t0301_bayarmasterlist.php";
		return $detailUrl;
	}
	...
	{
		return ($this->SqlFrom <> "") ? $this->SqlFrom : "`t0301_bayarmaster`";
	}
	...
	public function getModalCaption($pageName)
	{
		...
		elseif ($pageName == "t0301_bayarmasteredit.php")
			return $Language->phrase("Edit");
		elseif ($pageName == "t0301_bayarmasteradd.php")
	...
	{
		if ($parm <> "")
			$url = $this->keyUrl("t0301_bayarmasterview.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("t0301_bayarmasterview.php", $this->getUrlParm(TABLE_SHOW_DETAIL . "="));
	...
	{
		if ($parm <> "")
			$url = "t0301_bayarmasteradd.php?" . $this->getUrlParm($parm);
		else
	...
	{
		if ($parm <> "")
			$url = $this->keyUrl("t0301_bayarmasteredit.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("t0301_bayarmasteredit.php", $this->getUrlParm(TABLE_SHOW_DETAIL . "="));
		return $this->addMasterUrl($url);
	}
	...
	{
		if ($parm <> "")
			$url = $this->keyUrl("t0301_bayarmasteradd.php", $this->getUrlParm($parm));
		else
			$url = $this->keyUrl("t0301_bayarmasteradd.php", $this->getUrlParm(TABLE_SHOW_DETAIL . "="));
		return $this->addMasterUrl($url);
	}
	...
	{
		return $this->keyUrl("t0301_bayarmasterdelete.php", $this->getUrlParm());
	}
